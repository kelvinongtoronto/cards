...
def flip_card(mx,my):
    try:
        ...
        if moused_card.is_face_up:
            ...
            source_area = pygame.Rect((width*13, height*(0 if moused_card.back=='XX' else 1)), (width, height))
...
def swap_cards(card1x,card1y,card2x,card2y):
    try:
        ...
        if card1.is_face_up:
            ...
            source_area1 = pygame.Rect((width*13, height*(0 if card1.back=='XX' else 1)), (width, height))
            
        if card2.is_face_up:
            source_area2 = pygame.Rect((width*rank2, height*suit2), (width, height))
        else:
            source_area2 = pygame.Rect((width*13, height*(0 if card2.back=='XX' else 1)), (width, height))
...
while True: # main game loop
    ...
    if not d.is_empty():
        ...
        if c.is_face_up:
            ...
            source_area = pygame.Rect((width*13, height*(0 if c.back=='XX' else 1)), (width, height))
